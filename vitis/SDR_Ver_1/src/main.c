...
int main(void)
{
    ...
    while (1) {
        ...
        if (PacketLength == SDR_PACKET_BYTES) {
            ...
            xil_printf("Bad SG packet length: %u\r\n",
...
static void PrintDdcStatus(const char *Tag)
{
    ...
    FifoLevel = (Status >> 16) & 0x1fffU;
    PacketCount = Counters & 0xffffU;
    ClipCount = Counters >> 16;

    xil_printf("[%s] DDC status=0x%08x fifo=%u full=%u full_seen=%u stall_seen=%u clip_seen=%u packets=%u clips=%u\r\n",
               Tag,
               (unsigned int)Status,
